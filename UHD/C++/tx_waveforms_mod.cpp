...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    //}
    //for (size_t n = 0; n < buff.size(); n++) {
    //	buff[n] = std::complex<float>(buff[n].real(), 0.0);
    //}
    //
    // // //    
    //
    // Zero the necessary elements for the PDM pulse
    //   // for (size_t n = 18 - 0; n < 31 + 0; n++)
    //for (size_t n = 12; n < 37; n++) {
    //    buff[n] = std::complex<float>(0.0, 0.0);
    //}
    //
    //for (size_t n = 12 + 50; n < 37 + 50; n++) {
    //    buff[n] = std::complex<float>(0.0, 0.0);
    //}
    //
    ...
    if (encode_negative_halfperiod) {
    	...
    	for (size_t n = half_period + offset_left; n < half_period + offset_right; n++) {
        	buff[n] = std::complex<float>(pdm_null, pdm_null);
    	}
    }
    //
    ...
    std::cout << "The generated buffer is (as float): " << std::endl;
    for (size_t n = 0; n < buff.size(); n++) {
	std::cout << "buff[" << n << "] = " << "std::complex<float>(" << buff[n].real() << "," << buff[n].imag() << ");" << std::endl;
    }

    std::cout << "************************************" << std::endl;

    // some log on the console
    std::cout << "The generated buffer is (as short): " << std::endl;
    for (size_t n = 0; n < buff.size(); n++) {
        short i_value, q_value;
        i_value = (short)(buff[n].real() * 100);
        q_value = (short)(buff[n].imag() * 100);
	std::cout << "buff[" << n << "] = " << i_value << "|" << q_value << ";" << std::endl;
    }
