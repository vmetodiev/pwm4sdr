...
#define CONFIG_REG ( 4 )
#define PWM_EXPECTED_MIN ( 20 )
#define PWM_EXPECTED_MAX ( 50 )
    ...
    bool continue_on_bad_packet = false)
{
    ...
    while (not stop_signal_called
        ...
        uint32_t ref_in_val = 95 & 0x000FFFF;                       // Ensure it is 16-bit long 
        regs->poke32(CONFIG_REG, ref_in_val);
        uint64_t read_reg_val =  (regs->peek64(0));

        uint32_t config_reg_value = read_reg_val >> 32;             // Write reg value - ref_in value is here
        uint16_t pwm_decoder_out = read_reg_val & 0x00FF;           // Read  reg value - decoder value is here
        
        if ( (pwm_decoder_out > PWM_EXPECTED_MIN) and (pwm_decoder_out < PWM_EXPECTED_MAX) ) // SW Filter
            std::cout << boost::format("config: 0x%08X, Decoded Symbol: S%hu\n") %config_reg_value % decode_symbol(pwm_decoder_out);
